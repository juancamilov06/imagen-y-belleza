...
function checkCode(code){
    $.ajax({
        url: 'http://' + ip + '/admin-hub/v1/items/find/' + code,
        type: 'get',
        beforeSend: function () {
            $.showLoading({
                name: 'square-flip'
...
function createProduct(){
    ...
    if (empty(name) || empty(minimum) || empty(price) || price < 100 || empty(iva) || empty(payment_one) || empty(payment_two) || empty(payment_three)
        || empty(payment_four) || empty(client_one) || empty(client_two) || empty(client_three) || empty(client_four) || empty(client_five) || minimum < 0
            ){
        ...
        var item = {
            ...
            'discount_one':client_one,
            'discount_two':client_two,
            'discount_three':client_three,
            'discount_four':client_four,
            'discount_five':client_five,
            'is_active': 1,
            'is_new':1,
            'brand_id': brand_id,
        ...
        $.ajax({
            url:   'http://' + ip + '/admin-hub/v1/items/new',
            data: params,
            type:  'post',
            beforeSend: function(){
                $.showLoading({
...
function getData(){
    ...
    $.ajax({
        url:   'http://' + ip + '/admin-hub/v1/brands/all',
        type:  'get',
        beforeSend: function(){
            $.showLoading({
                name: 'square-flip'
...
function getSession(){
    ...
    if (localStorage.getItem("role") != 'Admin') {
        ...
        window.location.replace("/admin-hub/v1/main");
    };
}