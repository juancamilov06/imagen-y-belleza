...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public long usersCount() {
        SQLiteDatabase database = this.getWritableDatabase();
        SQLiteStatement statement = database.compileStatement("SELECT count(*) FROM " + Database.TABLE_USER);
        return statement.simpleQueryForLong();
    }

    public long brandsCount() {
        SQLiteDatabase database = this.getWritableDatabase();
        SQLiteStatement statement = database.compileStatement("SELECT count(*) FROM " + Database.TABLE_BRAND);
        return statement.simpleQueryForLong();
    }

    public long clientCount() {
        SQLiteDatabase database = this.getWritableDatabase();
        SQLiteStatement statement = database.compileStatement("SELECT count(*) FROM " + Database.TABLE_CLIENT);
        return statement.simpleQueryForLong();
    }

    public long orderCount() {
        SQLiteDatabase database = this.getWritableDatabase();
        SQLiteStatement statement = database.compileStatement("SELECT count(*) FROM " + Database.TABLE_ORDER);
        return statement.simpleQueryForLong();
    }

    public long orderItemCount() {
        SQLiteDatabase database = this.getWritableDatabase();
        SQLiteStatement statement = database.compileStatement("SELECT count(*) FROM " + Database.TABLE_ORDER_ITEMS);
        return statement.simpleQueryForLong();
    }

    public void deleteTempOrderStates(int orderId) {
        SQLiteDatabase database = this.getWritableDatabase();
        database.execSQL("DELETE FROM " + Database.TABLE_TEMP_ORDER_STATE + " WHERE " + Database.KEY_ID + " = " + orderId);
    }
...
*/
    ...
    public boolean insertItems(List<Item> items) {
        ...
        try {
            for (Item item : items) {
                ...
                values.put(Database.KEY_PRICE, item.getPrice());
                ...
                values.put(Database.KEY_DISCOUNT_ONE, item.getDiscountOne());
                values.put(Database.KEY_DISCOUNT_TWO, item.getDiscountTwo());
                values.put(Database.KEY_DISCOUNT_THREE, item.getDiscountThree());
                values.put(Database.KEY_DISCOUNT_FOUR, item.getDiscountFour());
                values.put(Database.KEY_DISCOUNT_FIVE, item.getDiscountFive());
    ...
    public double getParentPrice(int id) {
        SQLiteDatabase database = this.getWritableDatabase();
    ...
    public Item getItem(int id) {
        ...
        try {
            if (cursor.moveToFirst()) {
                ...
                item.setBrand(getBrand(cursor.getInt(5)));
                item.setCategory(getCategory(cursor.getInt(6)));
                item.setSubItemId(cursor.getInt(7));
                item.setDiscountOne(cursor.getDouble(8));
                item.setDiscountTwo(cursor.getDouble(9));
                item.setDiscountThree(cursor.getDouble(10));
                item.setDiscountFour(cursor.getDouble(11));
                item.setDiscountFive(cursor.getDouble(12));
                item.setPaymentOne(cursor.getDouble(13));
                item.setPaymentTwo(cursor.getDouble(14));
                item.setPaymentThree(cursor.getDouble(15));
                item.setPaymentFour(cursor.getDouble(16));
                item.setIva(cursor.getDouble(17));
            }
            cursor.close();
            return item;
        } catch (Exception e) {
